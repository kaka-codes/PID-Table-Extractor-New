...
def _get_full_width_top_clip_rect(pdf_bytes: bytes, page_index: int):
    """Return a clip rectangle covering the full page width and only the top band."""
    document = fitz.open(stream=pdf_bytes, filetype="pdf")
    try:
        page = document[page_index]
        rect = page.rect
        ...
        return fitz.Rect(
            ...
            0,
            rect.width,
            min(rect.height, DEFAULT_TOP_CLIP_HEIGHT),
        )
    finally:
        document.close()
...
def _ocr_page_result(pdf_bytes: bytes, filename: str, page_index: int) -> Dict[str, Any]:
    clip_rect = _get_full_width_top_clip_rect(pdf_bytes, page_index)
